...
###卡文件过多，在获取歌曲列表的时候，不能操控的问题
* 问题现象：
在获取蓝牙设备的卡音乐时，如果卡里面有大量的歌曲，比如一千多首歌曲，需要加载很久很久。在加载过程中长时间不能操作，这样造成用户体验比较差。   

* 问题原因：   
  在加载歌曲过程中，不能设置卡管理器的代理。如果设置，容易导致设备蓝牙崩溃，或者程序错乱。
  代码如下：
  
```
[LMBluetoothManager sharedInstance].m_CardMusicManager = [[LMBluetoothManager sharedInstance].m_MediaManager getMusicManager:self];

```

* 解决办法：
 在获取设备歌曲的时候，避免设置该卡管理器的代理。若需要用到该管理器的部分代理回调，需要通过当前代理传递给需要的界面。
